...
To perform this shift, the non-zero coefficients $\alpha_{i}^{(i+1)}, \dots, \alpha_i^{(j)}$ encoded in the boundary chains $r_{i+1}, \dots, r_j$ must be set to zero, otherwise $\partial(\sigma_i)$ contributes to earlier simplices in the filtration. 
As these coefficients are recorded by $v_j$, setting $\alpha_i^k = 0$  amounts to setting $v_k(i) = 0$ for all $k \in [i+1,j]$---algorithmically, these manifest as column operations $\ast$ used to ensure $PV\ast$ is upper-triangular, thus maintaining invariant I2.
Notice that, to maintain invariant I1, these operations induce a potentially unreduced $R' = P R \ast P^T$, breaking invariant I3.
We could reduce $R'$ with an additional $k$ operations, however $k$ may scale quadratically in the interval $\lvert i - j\rvert$, rendering the efficiency of this approach at best that of vineyards and at worse that of the standard algorithm. 
...
\begin{proposition}[Busaryev et al.~\cite{busaryev2010tracking}]
	Let $K$ denote a filtration with $n$ simplices of dimension $p-1$, $p$, and $p+1$, respectively, and let $R = DV$ denote its associated decomposition. Then the operation $\mathrm{MoveRight}(i,j)$ constructs a valid decomposition $R' = D'V'$ in $O((s+t)n)$ time, where $s,t$ are given by:
	$$ s = \sum\limits_{l=i+1}^j \mathds{1}\left(\, v_l(i) \neq 0 \, \right), \quad t = \sum\limits_{l=1}^m \mathds{1}\left(\, \mathrm{low}_R(l) \in [i,j] \text{ and } r_l(i) \neq 0 \, \right)$$
	Moreover, the quantity $k = s + t$ satisfies $k \leq 2(j - i)$. 
\end{proposition}
...
\subsubsection{Continuous setting}\label{sec:continuous_setting}
...
\end{equation}
This implies the $\lvert S_F \rvert \sim O(m^2)$ in the worst case, achieved when $f = - f'$, which is exhibited in the grayscale image data example in section~\ref{sec:motivation}: each 9x9 patch contains $(81, 208, 128)$ simplices of dimensions $(0, 1, 2)$ yielding $m=417$ simplices in total. As the homotopy is simulated, observe $\approx 70,\!000$ transpositions are generated. The upper bound size on $\lvert S_F \rvert$ is thus $\binom{417}{2} \approx 87,\!000$, which nearly occurs in this case, since the last filtration is nearly the reverse of the initial one.   
...
Contrasting the continuous-time setting, if we discard the use of a homotopy interpolation and allow move operations in any order, we have $\widetilde{S} \sim O(m)$ trivially: simply move each simplex in $K$ into its position in the filtration given by $K'$ in the order given by the latter. However, it's not immediately clear whether this bound is tight, or whether this strategy of generating schedules---which we call \emph{naive strategy}---is performant.  
%Indeed, arbitrarily permuting a given filtration $K$: one must respect the face poset.


To establish this formally, we require a few definitions. Given two fixed permutations $p, q \in S_m$ and a set $\Sigma \subseteq S_m$,  a common  problem is to find a sequence of permutations $s_1, s_2, \dots, s_d \in \Sigma$ whose composition satisfies:
...
Here we consider sorting by moves. Phrased using permutations, a \emph{move operation} $m_{ij}$ that moves $i$ to $j$ in $[m]$, for $i < j$, corresponds to the permutation: 
