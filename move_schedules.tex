...
When $d \sim O(m)$, the quadratic nature of the aforementioned greedy minimization can be computationally expensive. 
In order for Algorithm~\ref{alg:schedule} to be considered as an efficient alternative to vineyards---which requires $O(m \log m + k)$ preprocessing time to compute $k \sim O(m^2)$ transpositions---a solution which is subquadratic in $m$ is desirable. 
