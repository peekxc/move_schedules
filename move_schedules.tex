...
Indeed, Property M3 directly follows from this fact, as one may simply move each simplex $\sigma \in K$ into its order in $K'$ via insertion sort. Note that the number of valid decomposition produced by vineyards is bounded above by $O(m^2)$, since each pair of simplices $\sigma_i, \sigma_j \in K$ switches its relative ordering at most once is the homotopy assumptions from~\ref{sec:vineyards} hold.
