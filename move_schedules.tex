...
$$ \mathcal{B}(M) = \{ \; B_L(M) : L \in \mathbb{R} \times \mathbb{R}^{+} \; \}$$
	Thus, for a bipersistence modules $M$, the fibered barcode is simply a 2-parameter family of barcodes given by restricting $M$ to a set affine lines in $\mathbb{R}^2$. 
\end{definition} 
\noindent Each line $L$ induces a 1-parameter restriction on $M$. The fibered barcode enjoys a number of stability properties readily expressed using interleaving distances~\cite{landi2014rank}. 
