...
% With the current state of the art tools, there is no recourse to the problem of computing persistence from a time-varying filtration containing millions of simplices across thousands of snapshots in time.
...
There are two events which dramatically alter the persistence diagrams extracted from these filtrations: the first occurs when the central connected component splits to form a cycle, and the second  when the annulus  splits into four components. As a consequence, in this example, only a few persistence diagrams are needed to capture the major topological changes over time. 
  ...
  On the left side of Figure~\ref{fig:vineyards}, we compare the cumulative cost (in total column operations) of various approaches.
  Since it is unknown a priori at when the persistent pairing function changes, one solution is to discretize the time domain at $n$ evenly spaced points and compute the diagrams at each time point independently. 
...
Given a filtered simplicial complex $K$ with filtration function $f : K \to [m]$, denote by $R = D V$ the decomposition of its corresponding boundary matrix $D$ such that $R$ is reduced and $V$ is upper-triangular (see section~\ref{sec:reduction} for details).
 If one has a pair of filtrations $(K, f)$,  $(K', f')$ of equal size $m = \lvert K \rvert = \lvert K' \rvert$ and $R = D V$ has been computed for $K$, then it may be advantageous to use the information stored in ($R$, $V$) to reduce the computation of $R' = D' V'$. 
...
The pseudocode is outlined in Algorithm~\ref{alg:reduce} in the appendix. Without optimizations, like   clearing   or       implicit matrix reduction, the standard  algorithm is very inefficient. Nonetheless, it serves as the foundation of most persistent homology implementations, and its invariants are necessary before introducing both vineyards in section~\ref{sec:vineyards} and our move schedules  in section~\ref{sec:move_schedules}.
 ...
 Given a filtration $K$ with $m$ simplices, one assembles the elementary boundary chains $\partial(\sigma)$ as columns ordered according to the filtration into an $m \times m$  \emph{filtration boundary matrix} $D$. 
 It is at times more succinct to restrict to specific sub-matrices $D$ based on the homology dimension $p$, and so we write $D_p$ represent the $d_{p-1} \times d_p$ matrix representing the linear operator $\partial_p(K)$ (the same notation is extended to $R$ and $V$).
% The reduction algorithm computes the persistent homology from $D$ for all dimensions up to $\mathrm{dim}(K) - 1$.
%  Alternatively, a single dimension $p \leq \mathrm{dim}(K) - 1$ can be computed, in which case one reduces a pair of sub-matrices $D_p$ and $ D_{p+1}$ of $D$,  where $D_p$ has dimension $d_{p-1} \times d_p$ and $D_{p+1}$ is $d_{p} \times d_{p+1}$. 
%In some situations the description of the ensuing algorithms is simpler when considering all dimensions simultaneously, while in others it is more succinct to consider dimension-specific matrices. 
%In what follows $D$, $R$, and $V$ will always denote square $(m \times m)$ matrices, though we note that all algorithms discussed generalize to both cases. 
...
Using $\mathbb{Z}_2$ coefficients and homology in dimension 1 for simplicity, the reduction proceeds as follows:
\begin{displaymath}
	\def\rowsp{0.4em}
...
Nonetheless, many variations and optimizations to Algorithm~\ref{alg:reduce} have been proposed over the past decade, see~\cite{bauer2017phat, bauer2021ripser, chen2011persistent} for an overview. Despite these variations in computation, the resulting decomposition must satisfy the following invariants:
  \vspace*{0.8em}
  \\
 ...
 \begin{enumerate}[labelsep=3pt, topsep=2pt,itemsep=-0.25ex,parsep=1.0ex]
 	...
 	\item[l3.] $R$ is \emph{reduced}: if both $\mathrm{col}_i(R) \neq 0$ and $\mathrm{col}_j(R) \neq 0$, then $\mathrm{low}_R(i) \neq \mathrm{low}_R(j)$ 
 	\end{enumerate} 
 \vspace*{1em}
 If these two invariants are satisfied, then we call the decomposition \emph{valid}. 
...
%%\end{proposition}
%\noindent
Though the matrices $R$ and $V$ are not unique, it has been shown that the collection of persistent pairings (derived from $R$) are~\cite{zomorodian2005computing}. 
%When reducing the boundary matrix, we can therefore perform column operations in any order, as long as columns are added from left to right, and so long as every column operation performed in $R$ is also performed on $V$. 
In particular, once $R$ is in reduced form, the persistence diagram of the corresponding filtration $K$ is given by lowest non-zero ``pivot'' entries in $R$.
...
These rules prescribe certain column and row operations to apply to a given matrix decomposition either before, during, or after each individual transposition to ensure it is a valid decomposition.   
...
For completeness, we recapitulate the moves algorithm from~\cite{busaryev2010tracking}. 
Let $K$ denote a filtration of size $m = \lvert K \rvert$ and $R = DV = [ r_1, r_2, \dots, r_m]$ its decomposition, where $r_j$ ($v_j$) will denote the $j$-th column of $R$ ($V$). By definition, if $r_j = D v_j = 0$, then the inclusion $K_{j-1} \hookrightarrow K_{j}$ introduced a new cycle and homology class of dimension $p = \mathrm{dim}(\sigma_j)$ whose generator is encoded by the non-zero coefficients $(\alpha_i^{(j)}, \alpha_{i+1}^{(j)}, \dots, \alpha_j^{(j)})$ via the combination: 
\begin{equation}
	r_j = D v_j = \alpha_i^{(j)} \cdot \partial(\sigma_{i+1}) + \alpha_{i+1}^{(j)} \cdot \partial(\sigma_{i+1}) + \dots + \alpha_{j}^{(j)} \cdot \partial(\sigma_{j}) = 0
\end{equation}
...
To perform this shift, these non-zero coefficients $\alpha_i^{(i)}, \alpha_{i}^{(i+1)}, \dots, \alpha_i^{(j)}$ encoded in the boundary chains $r_i, r_{i+1}, \dots, r_j$ must be set to zero, otherwise $\partial(\sigma_i)$ contributes to earlier simplices in the filtration. 
In matrix form, these coefficients are recorded by $v_j$, and thus setting $\alpha_i^k = 0$ for all $k \in [i,j]$ amounts to setting $v_i(i), v_{i+1}(i), \dots, v_{j}(i)$ to $0$.

Computationally, setting $v_i(i) = v_{i+1}(i) = \dots = v_{j}(i) = 0$ with column operations $\ast$ ensures $PV\ast$ is upper-triangular, thus maintaining invariant I2.
To maintain invariant I1, these operations induce a corresponding $R' = P R \ast P^T$, which may be unreduced (breaking invariant I3).
We could reducing $R'$ with an additional $k$ operations, however $k$ may scale quadratically in the interval $\lvert i - j\rvert$, rendering the efficiency of this approach at best that of vineyards and at worse that of the standard algorithm. 
...
In particular, if during the cancellation  of $v_i(i) = v_{i+1}(i) = 0$ the column $r_{i+1} \mapsto r_{i+1}'$ becomes unreduced, then the pivot $\mathrm{low}_R(i+1)$ becomes a \emph{free}, possibly becoming a pivot again in $r_{i+2}', \dots, r_j'$.  
Thus, if we save $r_{i+1}$ prior to modification to a \emph{donor column}, we may re-use or \emph{donate} its pivot entry to one of $r_{i+2}, \dots, r_j$. 
This process is repeated at most $j - i - 1$ times---by construction, the matrix $R'$ stays reduced during the movement in all except possibly for the $i$-th column, and since these operations are left-to-right $V'$ will be upper-triangular.
%We may achieve this via left-to-right column operations, as before, however this may leave several chains in $R$ unreduced. 
%To ameliorate this, observe that since the initial set of chains $r_i, r_{i+1}, \dots, r_j$ are already reduced, their pivots are unique---thus, we should expect these entries. 
%setting any individual $\alpha_i^(k) = 0$ for some $i < k \leq j$ has the effect of changing its pivot entry
% participates in a linear combination of another 
%By definition, the column $r_i$ 
\\
\\
...
%\end{remark}
Though similar to vineyards, move operations confer a few additional advantages. We note two convenient properties of moves:
\\	
...
Indeed, the number of intermediate persistence diagrams is bounded above by $O(m^2)$ using vineyards if each pair of simplices $\sigma_i, \sigma_j \in K$ switches relative ordering at most once in a given schedule of transpositions, whereas with move operations this bound clearly is at most $O(m)$.  

As a final note, we remark that the combination of \emph{MoveRight} and \emph{MoveLeft} enable the dynamic addition or deletion of simplices to the underlying complex $K$ (in fact this was the motivation for its conception). In particular, given $K$ and its decomposition $R = DV$, obtaining the decomposition $R' = D' V'$ of $K' = K \cup \sigma$ can be achieved by appending its requisite elementary chains to $D$ and $V$, reducing them, and then calling \emph{MoveLeft}$(m+1, i)$, where $i = f'(\sigma)$. Dually, deleting a simplex $\sigma_i$ may be achieved via \emph{MoveRight} by moving $i$-th to the end and dropping them. 
...
Each $f_i$ induces a bijection $f_i^\ast : [m] \to [m]$, or equivalently, a permutation of the index set $[m]$. For each pair of filtrations $(K_i, K_{i+1})$, we assign the simplices of $K_i$ labels from index set $[m]$ in filtration order, relabeling $K_{i+1}$ accordingly using $f_i$. 
    ...
    \begin{algorithmic}[1]
    	...
    	\Procedure{MoveSchedule}{$\mathcal{K} = ( K_1, K_2, \dots, K_n )$} 
    		...
    		\For{$i = 1$\textbf{ to }$n-1$}
    			...
    			\State $\mathcal{S} \gets \mathcal{S} \cup \mathrm{GreedySchedule}(q, \mathrm{lis}_q)$ \Comment{$O(d^2\log m)$}
    		\EndFor
...
In contrast, if we allow move operations in any order, we have $O(m)$ as a trivial upper bound: simply move each simplex in $K_0$ into its position in the filtration given by $K_1$ in the order given by the latter. However, it's not immediately clear whether this bound is tight. 

To establish this formally, we require a few definitions. Let $S_m$ denote the symmetric group  of   permutations of  $[m]$ under function composition $\circ$. Given two fixed permutations $p, q \in S_m$ and a set $\Sigma \subseteq S_m$,  a common  problem is to find an ordered sequence of permutations $s_1, s_2, \dots, s_d \in \Sigma$ whose composition transforms $p$ into $q$:
...
\end{equation}
Similarly, in the context of edit operations, observe that a move operation can be interpreted as a paired delete-and-insert operation, i.e. $m_{ij} = (\mathrm{ins}_j \circ \mathrm{del}_i)$, where $\mathrm{del}_i$ denotes the edit operation that deletes the character at position $i$  and $\mathrm{ins}_j$ similarly inserts the same character at position $j$. 
...
Then, the smallest size any move schedule $S^*$ transforming $K_0$ into $K_1$ is given by: 
...
If $K, K'$ are random distinct filtrations each of size $m$, then $d \sim m - \sqrt{m}$ in expectation, with probability $1$ as $m \to \infty$.
\end{corollary}
\begin{proof} \normalsize
...
However, Boissonnat~\cite{boissonnat2018efficient} prove the number of distinct filtrations built from $k$-dimensional simplicial complex $K$ with $m$ simplices and $t$ distinct filtration values is \emph{at least} $\floor{\frac{t+1}{k+1}}^m$. 
...
Before discussing the practice of constructing such a schedule, we conclude by combining the propositions we've given so far and present the main theorem of this effort. 
  \begin{theorem}\label{prop:sparsity_result}
   Given a pair of distinct filtrations $K, K'$ of a simplicial complex $S$ and a decomposition $R = DV$ of $K$, the size of a minimal sequence $\mathcal{S} = \left( s_1, s_2, \dots, s_d \right)$ of cyclic permutations $s_k = (i_k, j_k)$ satisfying~\eqref{eq:move_perm} and: 
   	\begin{equation}\label{eq:rv_seq}
   		R = D_0 V_0 \overset{s_1}{\to} D_1 V_{1} \overset{s_2}{\to} \dots \overset{s_d}{\to} D_d V_{d} = R'
   	\end{equation}
...
\end{proof}
 ...
 %% New section
\subsection*{Constructing schedules}
...
%Alternatively, given $\mathcal{L}$, the algorithm may be executed online (up to $d$ times) in $O(m \log m)$ time and $O(m)$ space.
...
Such a generalization has appeared naturally in many application contexts, showing potential as a tool for exploratory data analysis~\cite{lesnick2012multidimensional}. Indeed, one of the often noted drawbacks of persistence is its instability with respect to strong outliers, which can obscure the detection of significant topological structures~\cite{buchet2015topological}.
By filtering the data by density in tandem with the e.g. the ambient metric, multidimensional persistence in theory enables practitioner to handle outliers directly.

Unfortunately, unlike one-parameter persistence, there is no complete discrete invariant for multi-parameter persistence.
Instead, Lesnick et al~\cite{lesnick2015interactive} associate a variety of incomplete invariants to 2-parameter persistence modules: we focus here on the \emph{fibered barcode} invariant, defined as follows: 
...
Utilizing the equivalence between the rank and fibered barcode invariants, Lesnick and Wright~\cite{lesnick2015interactive} developed an elegant way of computing $\mathcal{B}(M)$ via a reparameterization using standard point-line duality. This clever technique effectively reduces the fibered barcode computation to a sequence of 1-D barcode computations at ``template points'' lying within the 2-cells of particular planar subdivision $\mathcal{A}(M)$ of the half-plane $[0, \infty) \times \mathbb{R}$. This subdivision may be obtained via arrangement of ``critical lines'' induced by the bigraded Betti numbers $\beta(M)$ of $M$.
...
Despite this significant barrier, there is room for optimism: in practice, the external stability result from~\cite{landi2014rank} justifies the use of a coarsening procedure which approximates the module $M$ with a smaller module $M'$ via a grid-like reduction, enabling practitioners to restrict the size of $\kappa$ to a relatively small constant. 
This in-turn dramatically reduces the size of $\mathcal{A}(M)$ and thus the number of barcode templates to compute. 
Moreover, the ordering of barcode templates given by the dual graph traversal implies that adjacent  templates should be relatively similar---so long as $\kappa$ is not too small---suggesting barcode computations may be productively shared between adjacent templates. 
...
This can be achieved by expressing the patch data as an expansion of a certain convenient basis of 8 non-constant eigenvectors, often called the Discrete Cosine Transform (DCT) basis~\cite{lee2003nonlinear}, shown below: 
%\vspace{-0.2em}
\begin{center}
...
\subsection{2D Persistence}\label{app:2d_pers}
