...
\end{equation}
where $\mathrm{cost}(S(\sigma) \circ p)$ measures the number of column operations require to execute $S(\sigma)$ on an existing decomposition. There are exponentially many ways to generate such a sorting $S$. 
...
For any sorting $S$ such that $s_{i}$ that increases the size of longest common subsequence between $s_{i-1} \circ \dots \circ s_1 \circ p$ and $q$. Since the complexity of the move operations discussed in section~\ref{sec:moves} depend on the size of the movement, it is advantageous to construct a schedule which minimizes this movement. 
...
We consider the following objective modified from equation~\ref{eq:schedule_opt}: 
...
\end{equation}
The minimizer of this optimization can be interpreted as a sorting which minimizes the net displacement amongst all possible sortings, in the $\ell_1$ sense. This minimization problem is related to a constrained version of the bipartite crossing minimization problem for $k$ sets of permutations, which is NP-hard for $k \geq 4$~\cite{biedl2009complexity}. As a result, we instead consider a greedy approach. A greedy strategy requires $O((d+(d-1)+(d-2)+\dots+1)m) \approx O(d^2 m)$ time to complete, since there are $O(d)$ symbols to choose from and minimizing $F(\hat{s}_i, \hat{s}_{i+1})$ at each step takes $O(md)$. Since $\mathcal{D}$ is fixed, a greedy strategy thus depends on our ability to quickly compute $F(\hat{s}_i, \hat{s}_{i+1})$.  
%\\
%\\
%\noindent
